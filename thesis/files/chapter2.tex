...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.45\textwidth]{img/kernel_trick.png}
    \hfill
    \includegraphics[width=0.45\textwidth]{img/kernel2.png}
    \caption{The kernel trick conceptually mapping input data to a higher-dimensional feature space.}
...
During the testing phase, new sequences must be projected using the exact same $m$ landmarks selected during training. The Nystr\"{o}m method establishes a fixed coordinate system where each of the $m$ dimensions represents the geometric similarity to a specific training landmark. 
Evaluating a new test sequence requires measuring its similarity against those exact same reference points to guarantee spatial consistency. If new landmarks were sampled at test time, the resulting feature space would be defined by a completely different set of basis vectors, and the geometric alignment with the training data would be lost. 
For this reason, both the training landmarks and the $W^{-1/2}$ matrix must be saved and reused to compute the explicit projection for any unseen data.
