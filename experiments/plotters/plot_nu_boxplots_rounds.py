...
def extract_aucs_for_dir(results_dir: str, nu_label: str, n_reads: int, df_records: list):
    ...
    for loo_dir in loo_dirs:
        ...
        # Multiround script: expects 2D (seqs x rounds)
        if healthy_scores.ndim == 1:
            healthy_scores = healthy_scores.reshape(-1, 1)
...
def main():
    parser = argparse.ArgumentParser(description="Plot Grouped Boxplots of AUCs across Nu values (Multiround)")
    parser.add_argument("--results-dirs", nargs="+", required=True, help="List of multiround result directories")
    parser.add_argument("--nu-labels", nargs="+", required=True, help="List of Nu labels corresponding to results-dirs")
    parser.add_argument("--n-reads", type=int, default=20000, help="Fixed number of reads to subsample (default: 20000)")
    parser.add_argument("--output", default="boxplot_nu_comparison_rounds.png", help="Output PNG path")
    ...
    # Create grouped boxplot with whiskers at 5th and 95th percentiles
    ax = sns.boxplot(
        data=df, 
        x="Aggregator", 
        y="AUC", 
    ...
    ax.set_title(f"AUC Distributions Across Rounds (N={args.n_reads} sequences)", fontsize=16, pad=15)
