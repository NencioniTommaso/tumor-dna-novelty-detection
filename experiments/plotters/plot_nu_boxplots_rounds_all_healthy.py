...
def extract_aucs_for_dir(results_dir: str, nu_label: str, n_reads: int, df_records: list):
    ...
    for loo_dir in loo_dirs:
        ...
        for hf in healthy_files:
            hs = load_subject_scores(hf)
            if hs.ndim == 1:
                hs = hs.reshape(-1, 1)
            healthy_scores_all.append(hs)
...
def main():
    parser = argparse.ArgumentParser(description="Plot Grouped Boxplots of AUCs across Nu values (Multiround, ALL Healthy)")
    ...
    parser.add_argument("--output", default="boxplot_nu_comparison_rounds_all_healthy.png", help="Output PNG path")
    ...
    ax.set_title(f"AUC Distributions Across Rounds - ALL Healthy (N={args.n_reads} sequences)", fontsize=16, pad=15)
