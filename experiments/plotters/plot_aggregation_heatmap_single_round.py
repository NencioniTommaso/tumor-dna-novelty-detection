...
For each LOO fold, each round, and each (aggregator, N) combination:
  1. Sub-sample N reads from the patient's 20 000 per-round scores.
  2. Aggregate the N scores into a single patient-level score.
  3. Compute AUC as the normalized Mann-Whitney U statistic
     (1 healthy_test vs. all tumors).
The heatmap has:
  - x-axis: N reads (1000, 5000, 10000, 20000, 50000)
  - y-axis: aggregation method
  - cell annotation: mean AUC ± std (across the 7 LOO folds)

Usage
-----
    python experiments/plot_aggregation_heatmap.py \
...
def load_subject_scores(csv_path: str) -> np.ndarray:
    ...
    Returns
    -------
    scores : np.ndarray, shape (n_sequences, n_rounds)
        Each column is one round of anomaly scores.
    """
...
def classify_subject(filename: str) -> str:
    """Return 'healthy', 'tumor', or 'healthy_train' from the filename.

    Convention: files are named like ``Healthy_7_scores_seed42.csv``
    or ``Colo_11_scores_seed42.csv``.
    The LOO fold directory name (e.g. ``LOO_Healthy_7``) tells us
    which healthy patient is the test subject.
    """
...
def auc_healthy_vs_sick(
    ...
    """AUC della classificazione sani-vs-malati usando lo score come
    discriminante.

    Computes AUC as the normalized Mann-Whitney U statistic via rank sums.
    Ties are handled with average ranks.

    Parameters
    ----------
    healthy_scores : np.ndarray
        Aggregated anomaly scores of the healthy patients.
    sick_scores : np.ndarray
        Aggregated anomaly scores of the sick (tumor) patients.

    Returns
    -------
    auc : float
        AUC ∈ [0, 1].  1 = perfect separation (all sick more anomalous).
    """
    ...
    # Rank di tutti gli score uniti, ties via average rank.
...
def compute_heatmap_data(loo_dirs: list[str]) -> tuple[np.ndarray, np.ndarray]:
    """Compute mean AUC and std for each (aggregator, N) combination.

    Returns
    -------
    mean_matrix : np.ndarray, shape (n_aggs, n_Ns)
    std_matrix  : np.ndarray, shape (n_aggs, n_Ns)
    """
    n_aggs = len(AGG_ORDER)
    n_ns = len(N_VALUES)

    # Collect AUCs: fold × round × agg × N
    all_aucs = {
    ...
    for loo_dir in loo_dirs:
        ...
        for f in csv_files:
            ...
            # healthy_train subjects (healthy but not LOO) are not used for AUC

        if test_healthy_file is None:
            print(f"  WARNING: LOO healthy subject '{loo_subject}' not found, skipping.")
            continue

        if not tumor_files:
            print(f"  WARNING: No tumor files found in {loo_dir}, skipping.")
            continue

        # Load all scores (should be 1D since there is only 1 round per fold)
        ...
        print(f"  Healthy test: {loo_subject}  ({n_seqs} seqs)")
        print(f"  Tumors: {', '.join(tumor_names)}")

        # No loop over rounds needed, just one round per fold.
        # For each N
        for n_reads in N_VALUES:
            # Sub-sample (or use all if n_reads >= n_seqs)
            if n_reads >= n_seqs:
            ...
            for agg_name in AGG_ORDER:
                ...
                # Aggregate patient-level score
                h_agg = np.array([agg_fn(h_sub)])
                t_aggs = np.array([agg_fn(ts) for ts in t_subs])

                # AUC via Mann-Whitney U (normalized rank-sum)
...
def plot_heatmap(
    ...
    """Create an annotated heatmap and save to disk."""
    ...
    # Color map: use a diverging or sequential colormap
    cmap = sns.color_palette("vlag", as_cmap=True)

    # Determine vmin/vmax for consistent coloring
    ...
    # Add black border to the heatmap axis
...
def main():
    parser = argparse.ArgumentParser(
        description="Heatmap of AUC (Mann-Whitney U) by aggregation method × N reads.",
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Path containing the LOO_* fold directories. "
            "e.g. results/loo_multiround_nystrom_001/m_1/k_6"
        ),
    )
    parser.add_argument(
        "--output",
        ...
        help=(
            ...
            "Default: <results-dir>/heatmap_aggregation_auc.png"
        ),
    )
    parser.add_argument(
        "--title",
    ...
    print(f"\n{'=' * 60}")
    print("  AUC Results (mean ± std across folds)")
    print(f"{'=' * 60}")
    ...
    # Plot
    output_path = args.output or os.path.join(args.results_dir, "heatmap_aggregation_auc.png")
    title = args.title or "AUC (Mann-Whitney U) by Aggregation × N Reads"
