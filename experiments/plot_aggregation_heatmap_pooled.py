"""
plot_aggregation_heatmap_pooled.py
...
For each LOO fold, each bootstrap iteration, and each (aggregator, N):
  1. Sub-sample N reads from the patient's pooled scores.
...
Usage
-----
    python experiments/plot_aggregation_heatmap_pooled.py \
...
N_BOOTSTRAP = 10  # Number of times to resample for std calculation
...
def compute_heatmap_data(loo_dirs: list[str]) -> tuple[np.ndarray, np.ndarray]:
    ...
    for loo_dir in loo_dirs:
        ...
        # Load all scores and flatten them across rounds
        healthy_scores = load_subject_scores(test_healthy_file).flatten()
        n_seqs_total = len(healthy_scores)
        ...
        print(f"  Healthy test: {loo_subject}  ({n_seqs_total} total fragments)")
        ...
        for b in range(N_BOOTSTRAP):
            for n_reads in N_VALUES:
                # Sub-sample healthy
                if n_reads >= n_seqs_total:
                    h_sub = healthy_scores
                else:
                    idx_h = rng.choice(n_seqs_total, size=n_reads, replace=False)
...
def plot_heatmap(
    ...
    output_path: str,
    title: str = "AUC (Mann-Whitney U) by Aggregation × N Reads (Pooled)",
) -> None:
    ...
    # Slightly wider figure for more N_VALUES
...
def main():
    parser = argparse.ArgumentParser(
        description="Heatmap of AUC using pooled scores from all rounds.",
    ...
    output_path = args.output or os.path.join(args.results_dir, "heatmap_aggregation_auc_pooled.png")
