...
def compute_heatmap_data(loo_dirs: list[str]) -> tuple[np.ndarray, np.ndarray]:
    ...
    for loo_dir in loo_dirs:
        ...
        # Load all scores (shape: n_seqs × n_rounds)
        healthy_scores = load_subject_scores(test_healthy_file)
        n_seqs, n_rounds = healthy_scores.shape
        ...
        print(f"  Healthy test: {loo_subject}  ({n_seqs} seqs × {n_rounds} rounds)")
        print(f"  Tumors: {', '.join(tumor_names)}")

        # For each round
        for r in range(n_rounds):
            healthy_round = healthy_scores[:, r]
