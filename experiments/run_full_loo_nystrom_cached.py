"""
run_full_loo_nystrom_cached.py
...
def main():
    parser = create_base_parser("Full LOO Experiment with Cached Nyström")
    ...
    base_out_dir = os.path.join(
        project_root, "results", "full_loo_nystrom_cached", 
    ...
    logger.info("=" * 65)
    logger.info(" FULL LOO EXPERIMENT (CACHED NYSTRÖM)")
    logger.info("=" * 65)
    ...
    for held_out_id in range(args.start_fold, 8):
        ...
        del X_norm_train
        gc.collect()
        logger.info(f"Row normalization + Nyström fit/transform complete in {time.time() - t0:.1f}s")
