...
def _read_and_track(file_list, desc, max_total_seqs, label, cache_dir, logger):
...
def load_tracked_patient_cohort(train_normal_files, test_normal_files, test_tumor_files, max_train: int, max_test_normal: int, max_test_tumor: int, seed: int, cache_dir: str, logger):
    ...
    train_data, _ = _read_and_track(train_normal_files, "Train (Normal)", max_train, None, cache_dir, logger)

    logger.info("\n--- Loading Testing Data (Tracked Instances) ---")
    test_normal_data, normal_info = _read_and_track(test_normal_files, "Test (Normal)", max_test_normal, 1, cache_dir, logger)
    test_tumor_data, tumor_info = _read_and_track(test_tumor_files, "Test (Tumor)", max_test_tumor, -1, cache_dir, logger)
...
def load_train_cohort_only(train_normal_files, max_train: int, seed: int, cache_dir: str, logger):
...
def load_test_cohort_only(test_normal_files, test_tumor_files, max_test_normal: int, max_test_tumor: int, seed: int, cache_dir: str, logger):
